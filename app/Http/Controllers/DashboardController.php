    ...
    public function index()
    {
        ...
        switch ($usuario->rol?->nombre ?? '') {
            case 'admin':
                ...
                $aprobadas = Solicitud::whereHas('estado', fn ($q) => $q->where('codigo', self::ESTADO_APROBADO))->count();
                $rechazadas = Solicitud::whereHas('estado', fn ($q) => $q->where('codigo', self::ESTADO_RECHAZADO))->count();
                $pendientes = Solicitud::whereHas('estado', fn ($q) => $q->where('codigo', self::ESTADO_PENDIENTE))->count();
            ...
            default:
                ...
                $rechazadas = $usuario->solicitudes()
                    ->whereHas('estado', fn ($q) => $q->where('codigo', self::ESTADO_RECHAZADO))
                    ->count();
