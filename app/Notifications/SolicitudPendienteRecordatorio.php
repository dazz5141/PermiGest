...
class SolicitudPendienteRecordatorio extends PermiGestNotification
{
    ...
    {
        $this->solicitud->loadMissing(['usuario', 'tipo']);

        return (new MailMessage)
            ->subject("PermiGest: solicitud #{$this->solicitud->id} pendiente de revision")
            ->greeting("Hola {$notifiable->nombre_completo}")
            ->line('Tienes una solicitud de permiso pendiente de revision.')
            ->line('Folio: #'.$this->solicitud->id)
            ->line('Solicitante: '.$this->solicitud->usuario->nombre_completo)
